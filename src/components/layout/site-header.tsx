...
import { BookOpen, Building, FilePlus } from "lucide-react";
import {
  Tooltip,
  TooltipTrigger,
  TooltipContent,
...
export function SiteHeader() {
  ...
  const [currentIcon, setCurrentIcon] =
    useState<React.ComponentType<{ className: string }>>(Building);
      ...
      <div className="flex w-full items-center gap-1 px-4 lg:gap-2 lg:px-6">
        <Tooltip>
          ...
          <TooltipContent side="right">
            {isMac ? (
              <kbd className="rounded bg-black px-1.5 py-0.5 text-xs font-mono font-semibold">
                ⌘ + B
              </kbd>
            ) : (
              <kbd className="rounded bg-black px-1.5 py-0.5 text-xs font-mono font-semibold">
